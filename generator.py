...
class Generator:
    ...
    def define_graph(self):
        ...
        with tf.variable_scope('G_network', reuse=tf.AUTO_REUSE):
            ...
            #self.kernel_assign_op = tf.assign(ref = tf.get_variable(name=pjname,
            #                                                        shape=projected.shape),
            #                                  value = self.initializer_value,
            #                                  name = 'G_kernel_assign_op')
            ...
            #self.output_mean = tf.reduce_mean(self.output, name='G_output_mean')
    ...
    def define_graph_postD(self):
        ...
        #self.loss = tf.reduce_mean(tf.subtract(onesrandom, tf.log(self.D.p_fake)), name='G_loss')
        #self.loss = -tf.reduce_mean(tf.log1p(self.D.p_fake), name = 'G_loss')
    ...
    #def optimize_initializer(self, session):
    #    '''Generator Networkのカーネル初期値を最適化する'''

    #bestscore = -1.0
    #    bestparam = -1.0

    #    while self.inival < 5000.0:
    #        fd = { self.initializer_value: np.full(shape = (cf.MINIBATCHSIZE, self.projectedunits), fill_value = self.inival, dtype = np.float) }
    #        _, output = session.run([self.kernel_assign_op, self.output], feed_dict = fd)
    #        score = np.std(output)
    #        logger.info('Init value: ' + str(self.inival) + ', Output stddev: ' + str(score))
    #        if score > bestscore:
    #            bestscore = score
    #            bestparam = self.inival

    #        self.inival += 12.5

    #    #最後に最適値でカーネルを初期化する
    #    session.run(self.kernel_assign_op, feed_dict = {
    #        self.initializer_value: np.full(shape = (cf.MINIBATCHSIZE, self.projectedunits),
    #                                        fill_value = bestparam, dtype=np.float) })
